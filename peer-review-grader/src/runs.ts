...
export class Run {
    ...
    loadProjectMapping(): Map<string, string> {
        let mapFilePath = this.path + '/mapping.json';
        let files = fs.readdirSync(this.path)
        if(!files.includes('mapping.json')) {
            return new Map();
        }
        const contents = fs.readFileSync(mapFilePath, 'utf-8');
        let mapping = new Map<string, string>(JSON.parse(contents));
        return mapping;
    }

    saveProjectMapping(mapping: Map<string, string>) {
        let mapFilePath = this.path + '/mapping.json';
        const contents = JSON.stringify([...mapping]);
        fs.writeFileSync(mapFilePath, contents);
    }
    saveScores(mapping: Map<string, number>, filename: string) {
        let mapFilePath = this.path + `/${filename}.json`;
        const contents = JSON.stringify([...mapping]);
        fs.writeFileSync(mapFilePath, contents);
    }
